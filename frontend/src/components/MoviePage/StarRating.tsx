  ...
  useEffect(() => {
    const fetchUserRating = async () => {
      try {
        ...
        const existingRating = await res.json();
        ...
        >
          <div
            dangerouslySetInnerHTML={{
              __html: `<svg width="24" height="24" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path fill-rule="evenodd" clip-rule="evenodd"
                  ...
                  fill="${(hovered !== null ? index <= hovered : index < rating) ? '#FFD700' : '#4A4A4A'}"
                  fill-opacity="0.84"
                ></path>
              </svg>`,
            }}
